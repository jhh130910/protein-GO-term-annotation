...
if($cls eq "IPR"){
	my $pairFile="$Bin/../data/ParentChildTreeFile.txt";
		if(defined $supplyF2){
			CompareTwolist_IPR($ipr2geneF,$supplyF,$supplyF2,$outDir,$p_adjust_method,$TestMethod,$filt,$pc);
		}else{
    	enrichIPR($ipr2geneF,$supplyF,$backF,$outDir,$p_adjust_method,$TestMethod,$filt,$pc,$Unanno,$pairFile);
		}
}
