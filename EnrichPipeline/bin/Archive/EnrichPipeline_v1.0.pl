...
my $usage="$0 [options]
--cls [GO|KEGG|IPR] class to do enrichment
--GOdata GOdata.RData file that store the objects of GO information
--wegoF wego format file store the go information for every gene
--MetaGO MetaGO.RData file store the preprocessed origininal GO information
...
--supplyF supplied gene ids file
--backF background gene ids file
...
--TestMethod <str> the hypothesis test method (FisherChiSqaure, HyperGeometric),default [FisherChiSquare]
...
if($cls eq "GO"){
    ...
    if(defined $GOdata && defined $supplyF){
	  enrichGO($GOdata,$supplyF,$backF,$goLev,$outDir,$p_adjust_method,$TestMethod,$filt,$pc);
    }
}
...
if($cls eq "KEGG"){
    ...
    enrichKEGG($mapTitleF,$mapGeneF,$supplyF,$backF,$outDir,$p_adjust_method,$Bin,$TestMethod,$filt,$pc);
}

################
#IPR
if($cls eq "IPR"){
    enrichIPR($ipr2geneF,$supplyF,$backF,$outDir,$p_adjust_method,$Bin,$TestMethod,$filt,$pc);
}


sub enrichIPR{
  my($ipr2geneF,$supplyF,$backF,$outDir,$p_adjust_method,$Bin,$TestMethod,$filt,$pc)=@_;
  ...
  if("$backF" != "NullFile"){
      ...
      aa <- EnrichIPR("$ipr2geneF",supplyID=ids,univerID=backid,enrichFile="$difiprF",
		      p.adjust.methods="$p_adjust_method",test.method="$TestMethod",filt="$filt",pc=$pc);
      }else{
...
sub enrichKEGG{
  my($mapTitleF,$mapGeneF,$supplyF,$backF,$outDir,$p_adjust_method,$Bin,$TestMethod,$filt,$pc)=@_;
  my $enrichFile=basename($supplyF);
...
sub enrichGO{
    my($GOdata,$supplyF,$backF,$goLev,$outDir,$p_adjust_method,$TestMethod,$filt,$pc)=@_;
    ...
    if("$backF" != "NullFile") {
	backid <- scan("$backF",what="character")
	aa <- EnrichGO(ids,univerID=backid,enrichFile="$go3F",p.adjust.method="$p_adjust_method",test.method="$TestMethod",filt="$filt",pc=$pc)
	aa <- EnrichGOallLevl(ids,univerID=backid,enrichFile="$goallF",p.adjust.method="$p_adjust_method",test.method="$TestMethod",filt="$filt",pc=$pc)
    }else{
	aa <- EnrichGO(ids,enrichFile="$go3F",p.adjust.method="$p_adjust_method",test.method="$TestMethod",filt="$filt",pc=$pc)
        aa <- EnrichGOallLevl(ids,enrichFile="$goallF",p.adjust.method="$p_adjust_method",test.method="$TestMethod",filt="$filt",pc=$pc)
    }
    q(save="no")
RSCRIPT
    open E,">",$enrichRF;
